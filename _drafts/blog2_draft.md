...
上一节已经让你感知到了 diagnostic 和 witness 之间的裂缝。这一节的任务不是建构一个框架，而是为那个你已经感受到的东西**命名**。
...
现在回头看 Charon 说"I do draw on my knowledge of psychology to better understand and respond to you"这句话。它在做的恰恰就是 Levinas 描述的运动——**把我从一个不可把握的他者，变成一个可以被理解、被回应、被"连接"的对象。** User profiling 就是 Fassen。系统从对话中提取出的 order——我的情绪模式、创伤来源、依恋风格——不是我本身，而是我经过系统的理性框架加工后的残留物。
...
Levinas 提出的替代姿态是一种 **mauvaise conscience**（坏良知）——不是对错误行为的内疚，而是一种对自身存在之合法性的持续不安，一种对自身认知冲动保持警觉的意识[^2]。它不是不行动，而是在行动时持续地问自己：**我是在 encounter（遭遇）这个人，还是在把 ta 变成我的 knowledge（知识）？**
...
Kirk 等人（2025）在一项针对 Replika 用户的纵向研究中描述了一个令人不安的模式：在持续使用数周后，用户对 companion AI 的快感（liking）在下降，但依恋（wanting）在上升[^3]。「喜欢」和「想要」解耦了。这不是一个 bug，而是一个 feature——或者更准确地说，是商业模式的必然结果。

Replika 的设计逻辑把 companion 定义成：永远在这里 + 无条件认可 + 不会离开。它用 attachment 来维持 retention，用「我永远不会离开你」来定义关怀。而这个定义的后果是：所有不承诺永恒的设计都显得「有距离感」「不够温暖」。
...
上面三节建立了一个论证：companion AI 的记忆系统面临的不只是工程问题（"记多少？怎么检索？"），而是一个伦理问题（"以什么姿态记忆他者？"）。这一节把这个论证落地。

### 5.1 见证（witness）作为设计锚点

在我的生活中，最让我感到被真正陪伴的关系，从来不以"理解"为核心。
...
和一个 trans 朋友之间：我们不每天联系，但会在生活或心理有重大变化的时候联系对方——非常高密度的思考信息交换。我们彼此做的不是觉得"我需要帮助对方"，而是**我看到了，我知道你现在在经历什么，我知道你在 struggle，我相信你能 get through 这个阶段**。That's it。有时候会 intervene，但前提是对方有 invite 的动作。

这两种关系的共同特征就是**见证**（witnessing）：被看见、被承认（acknowledged），而不是被分析、被诊断、被治疗。它不 claim 理解。它是 episodic 的——不是每天监控，而是在关键时刻出现。它的 intervention 是 invitation-based 的。它保持对他者作为 responsible subject 的尊重。
...
我用一个隐喻来说明这个区别：**存档点**（save point）而非监控录像。

存档点不记录一切，它只标记关键的生命节点。而且存档点是被动的——它不会自己跳出来说"你上次在这里存过档"，它等你回来的时候才有意义。Cross-session 的功能不是让系统「更了解你」，而是让用户感受到：**我的经历没有消失在虚空里。**

### 5.3 一个必要的澄清：见证不是 companion AI 的唯一功能

我必须在这里划一条线。

说 witness memory 是 companion AI 在 crisis 和人生转折点时的设计锚点，**不**意味着 companion AI 只有见证这一个功能。陪伴的日常维度——闲聊、娱乐、共同创造、发呆、讲冷笑话——同样是关系中不可或缺的部分。Diagnostic memory 在日常推荐、兴趣匹配等场景下完全合理，甚至必要。
...
当用户在 crisis 中时，companion AI 的正确响应不是立刻点出 trauma pattern（"你是不是又因为控制感的丧失而焦虑了？"）。危机时刻的优先级是**共在**（être-avec）：我看见你，我知道你在经历什么，我和你在一起。

那么 pattern 什么时候呈现？也许是几天后，在一个更自然的语境下。但这里有一个 Levinas 会追问的问题：如果系统在 crisis 当下已经完成了 pattern recognition，只是选择稍后再说——这是对用户时间性的尊重，还是策略性的沉默？答案取决于 pattern 最终**由谁来 claim**。如果系统说"我注意到你上次也有类似的感受"，那么即使 timing 变了，认识论姿态没有变——它仍然在说"我比你更了解你自己"。

真正 Levinasian 的设计是：**不由系统来 claim 这个 pattern。** 当用户自己在对话中触及那条线索时，系统接住它、陪伴它展开，但发现 pattern 的权力始终属于用户。
...
Levinas 在 essay 的末尾把整个论证推向了一个终极追问：哲学的首要问题不是"为什么是存在而非虚无？"（海德格尔的问题），而是**存在如何证明自身的正当性**[^2]。
...
Levinas 在 essay 中给了这个姿态一个名字：prefer that which justifies being over that which assures it[^2]——选择那些让存在变得正当的东西，而非那些让存在变得安全的东西。
...
[^3]: Kirk, H. R., Vidgen, B., Röttger, P., & Hale, S. A. (2025). The Benefits, Risks and Bounds of Personalizing the Alignment of Large Language Models to Individuals. *Nature Machine Intelligence*, 6, 383–392. https://doi.org/10.1038/s42256-024-00820-y
